...
function InvoiceForm({ action, eligible, token }: { action: Action; eligible: InterventionView[]; token: string }) {
  ...
  return <form action={formAction} className="mt-5 rounded-md border border-dashed border-slate-300 bg-slate-50 p-4">
    ...
    <div className="mt-3 space-y-2">
      {eligible.map((item) => <label className="flex items-start gap-2 rounded border border-slate-200 bg-white p-3 text-sm" key={item.id}>
        <input
          className="mt-1"
    ...
    <Message state={state} />
    <button className="mt-3 rounded-md bg-[var(--brand)] px-4 py-2 text-sm font-semibold text-white disabled:opacity-60" disabled={pending || selected.length === 0} type="submit">{pending ? "Émission…" : "Émettre la facture"}</button>
  </form>;
}
...
function InvoiceCard({ invoice, patientId, role, voidAction }: { invoice: InvoiceSummary; patientId: string; role: AppRole; voidAction: Action }) {
  ...
  return <article className="rounded-md border border-slate-200 p-4">
    ...
    <Link className="mt-3 inline-flex text-sm font-semibold text-[var(--brand-strong)]" href={`/patients/${patientId}/invoices/${invoice.id}/print`}>Imprimer la facture</Link>
    {role === "doctor" && invoice.status === "active" ? <form action={formAction} className="mt-3" onSubmit={(event) => { if (!confirm("Annuler cette facture ? Cette action n’efface pas la facture.")) event.preventDefault(); }}>
      <label className="text-sm font-medium">Motif d’annulation<input className={input} maxLength={500} minLength={5} name="reason" required /></label>
      <p className="mt-2 text-xs text-[var(--muted)]">Cette action n’efface pas la facture et son numéro ne sera jamais réutilisé.</p>
      <Message state={state} />
      <button className="mt-2 text-sm font-semibold text-red-700 disabled:opacity-60" disabled={pending} type="submit">Annuler la facture</button>
    </form> : null}
  </article>;
}
...
function PaymentReceiptCard({ action, patientActive, patientId, payment, receipt }: { action: Action; patientActive: boolean; patientId: string; payment: Payment; receipt?: ReceiptSummary }) {
  ...
  return <article className="rounded-md border border-slate-200 p-4">
    ...
    {receipt ? <Link className="mt-3 inline-flex text-sm font-semibold text-[var(--brand-strong)]" href={`/patients/${patientId}/receipts/${receipt.id}/print`}>Imprimer le reçu</Link> : payment.status === "received" && patientActive ? <form action={formAction} className="mt-3"><Message state={state} /><button className="text-sm font-semibold text-[var(--brand-strong)] disabled:opacity-60" disabled={pending} type="submit">{pending ? "Génération…" : "Générer le reçu"}</button></form> : <p className="mt-2 text-xs text-[var(--muted)]">Aucun nouveau reçu ne peut être émis pour ce paiement.</p>}
  </article>;
}
...
function Message({ state }: { state: FinancialDocumentActionState }) { return state.message ? <p aria-live="polite" className={`mt-3 text-sm ${state.success ? "text-emerald-700" : "text-red-700"}`}>{state.message}</p> : null; }
