...
alter table public.interventions add column idempotency_key uuid;

-- Existing rows receive their own immutable UUID. This preserves every existing
-- intervention identifier and relationship without inventing a shared key.
...
alter table public.interventions
  ...
  add constraint interventions_idempotency_key_key unique (idempotency_key);

comment on column public.interventions.idempotency_key is
  'Opaque request key used to make intervention creation replay-safe.';
...
begin
  ...
  select * into existing from public.interventions
  where idempotency_key = target_idempotency_key;
  if found then
    select coalesce(array_agg(tooth_number order by tooth_number), '{}'::smallint[])
    into stored_teeth from public.intervention_teeth
    where intervention_id = existing.id;
    select coalesce(array_agg(dental_finding_id order by dental_finding_id), '{}'::uuid[])
    into stored_findings from public.intervention_findings
    where intervention_id = existing.id;
  ...
  select intervention_id, t from unnest(requested_teeth) t;
  insert into public.intervention_findings
  select intervention_id, f from unnest(requested_findings) f;
  return intervention_id;
end
$$;

revoke all on function public.create_intervention(
  uuid,date,text,numeric,public.intervention_status,text,smallint[],uuid[]
